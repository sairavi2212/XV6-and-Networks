...
void deque(queue *a)
{
  ...
  {
    a->queue_proc[i] = a->queue_proc[i + 1];
  }
  a->end--;
  return;
}

void enqueue(queue *a, struct proc *x)
{
  if (a->end == NPROC)
  {
    panic("Error!");
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        enqueue(&mlfq[p->queue_num], p);
        // printf("queue is %d\n",p->queue_num);
      }
      release(&p->lock);
    }
    struct proc *schedule = 0;
    ...
    for (int i = 0; i < 4; i++)
    {
      if (size(&mlfq[i]))
      {
        for (int j = 0; j < NPROC; j++)
        {
          if (mlfq[i].queue_proc[j] != 0)
          {
            p = mlfq[i].queue_proc[j];
            acquire(&p->lock);
            if (p->state == RUNNABLE)
            {
              if (leasttime > p->arrival_time_queue)
              {
                leasttime = p->arrival_time_queue;
                schedule = p;
              }
            }
            release(&p->lock);
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    // Disable interrupts to avoid conflicts while scheduling
    intr_on();

    // Calculate the total number of tickets for all RUNNABLE processes
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
      if (p->state == RUNNABLE)
    ...
    // If no runnable processes exist, return
    if (total_tickets == 0)
      continue;

    // Pick a winning ticket randomly between [0, total_tickets)
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        if (current_tickets >= winning_ticket)
        {
          ...
          {
            winner = p; // Select the process with the winning ticket or tie-break by arrival time
          }
        }
      }
      release(&p->lock);
    ...
    if (winner != 0)
    {
      // Context switch to the winner process
      acquire(&winner->lock);
